...
// 翻转操作
void bit_opposite(char *array,int index){
  index/=2;
  char m = (1 << (index % 8));
  array[index/8] ^= m;
}
...
int bit_alloc_isset(char *array,int index){
  index/=2;
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_opposite(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bit_opposite(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_opposite(bd_sizes[k].alloc, bi);  // free p at size k
    if (bit_alloc_isset(bd_sizes[k].alloc, buddy)) {  // 如果异或是1，且bi free,则buddy allocated
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      bit_opposite(bd_sizes[k].alloc,bi);
    }
  }
} 
...
bd_initfree_pair(int k, int bi,void *left,void *right) {
  ...
  if(bit_alloc_isset(bd_sizes[k].alloc,bi)){
    // one of the pair is free
    free = BLK_SIZE(k);
    if((addr(k,bi)>=left)&&((addr(k,bi)+free)<right)){//如果add(k,bi)在有效区域内，则bi free反之亦然
