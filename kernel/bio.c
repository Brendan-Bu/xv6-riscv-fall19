...
bget(uint dev, uint blockno)
{
  ...
  // 如果在h对应的bucket中没有找到，在其他bucket中找到buffer后，还要将其插入到原bucket中
  int next=(h+1)%NBUCKETS;
  while(next!=h){// 当next重新变成h，说明所有的buffer都refcnt不为0,panic
    acquire(&bcache.lock[next]);
    for(b = bcache.hashbucket[next].prev; b != &bcache.hashbucket[next]; b = b->prev){
      if(b->refcnt == 0) {
        b->dev = dev;
        b->blockno = blockno;
        b->valid = 0;
        b->refcnt = 1;
        // 从原来bucket的链表中断开,头插法插入到blockno对应的bucket中去
        b->next->prev=b->prev;
        b->prev->next=b->next;
